...
export const getOutOfStockProducts = () => async (dispatch) => {
    try {
      ...
      const response = await fetch("/api/v1/admin/products/outofstock");
      const data = await response.json();
  
      dispatch(outOfStockSuccess({ products: data.products }));
    } catch (error) {
      dispatch(outOfStockFail(error.response.data.message));
    }
  };